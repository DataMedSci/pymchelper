...
class SHTripCubeWriter:
    ...
    def write(self, detector):
        ...
        if detector.dettyp == SHDetType.dose:
            ...
            # .dos dose cubes are usually in normalized integers, where "1000" equals 100.0 % dose.
            # the next are also the defaults, but just to be clear this is specifially set.
        ...
        elif detector.dettyp in (SHDetType.dlet, SHDetType.tlet, \
            ...
            # When https://github.com/pytrip/pytrip/issues/35 is fixed, then this should not be needed.
            cube.cube = np.ones((cube.dimz, cube.dimy, cube.dimx), dtype=cube.pydata_type) * (1.0)
            
            cube.cube = detector.data.reshape(detector.nx, detector.ny, detector.nz)
            cube.cube = cube.cube * 0.1 # MeV/cm -> keV/um

            cube.write(self.output_corename)
            
        else:
            import sys
            sys.stderr.write("ERROR: can only write dose- or LET-type detectors to tripcube target.\n")
