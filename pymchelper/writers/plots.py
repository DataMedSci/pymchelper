...
class ImageWriter:
    ...
    def write(self, detector):
        ...
        if detector.dimension == 1:
            ...
            xr = [plot_x_axis.min_val, plot_x_axis.max_val]
            yr = [plot_y_axis.min_val, plot_y_axis.max_val]
            ...
            im = ax.pcolorfast(xr, yr, zlist, cmap=self.colormap, norm=norm)
        ...
        if detector.dimension == 2 and not np.all(np.isnan(error_raw)) and np.any(error_raw):
            ...
            self._save_2d_error_plot(detector, xr, yr, elist, x_axis_label, y_axis_label, z_axis_label)
