...
class SHReader(Reader):
    ...
    def corename(self) -> str:
        ...
        #  - corenameABCD.bdo or corename_ABCD.bdo (where ABCD is 4-digit integer)
        #  - corename.bdo

        core_name = base_name
        if len(base_name) > 4 and base_name[-4:].isdigit():
            core_name = base_name[:-4].rstrip("_")
...
def _postprocess(estimator: Estimator, nscale: float):
    ...
    if nscale != 1:
        ...
        for page in estimator.pages:
            ...
            if page.dettyp == SHDetType.dirtydose:
                page.dettyp = SHDetType.dirtydosegy
            # for the same reason as above we change units
            if page.dettyp in (SHDetType.dose_gy_bdo2016, SHDetType.alanine_gy_bdo2016, SHDetType.dirtydosegy):
