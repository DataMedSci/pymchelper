...
def reader_factory(filename):
    if file_has_sh_magic_number(filename):
        reader = _SHReaderBDO2019(filename)

        file_format = read_token(filename, SHBDOTagID.format)
        if file_format:
            ...
            if file_format == SHFileFormatId.bdo2019:
                ...
                reader = _SHReaderBDO2016(filename)
            else:
        ...
        reader = _SHReaderBin2010(filename)

    ver_short = extract_sh_ver(filename)
    logger.info("Short version: {:s}".format(str(ver_short)))
    ver_long = read_token(filename, SHBDOTagID.shversion)
...
class SHReader:
    ...
    def __init__(self, filename):
        ...
        self.reader_obj = reader_factory(self.filename)
        self.test_new_bdo_format()


    def read(self, detector, nscale=1):
        self.reader_obj.read(detector)

        # if self.test_version_0p6():
        #     if self.test_new_bdo_format():
        #         reader = _SHReaderBDO2019(self.filename)
        #     else:
        #         reader = _SHReaderBDO2016(self.filename)
        #     reader.read(detector)
        # else:
        #     reader = _SHReaderBin2010(self.filename)
