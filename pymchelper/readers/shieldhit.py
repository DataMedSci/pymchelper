...
class SHBinaryReader:
    ...
    def test_new_bdo_format(self):
        '''
        :return: True if SHIELD-HIT12A produced new BDO file format
        '''
        ...
        with open(self.filename, "rb") as f:
            ...
            while f:
                ...
                logger.info("Testing, reading id={:d}, {} ".format(pl_id, _pl))
                ...
                logger.debug("Read token {:s} (0x{:02x}) value {} type {:s}".format(
                    SHBDOTagID(pl_id).name,
                    pl_id,
                    _pl,
                    _pl_type.decode('ASCII'),))

                ## check if estimator type is defined
    ...
    def test_version_0p6(self):
        '''
        :return: True if SHIELD-HIT12A version is newer than 0.6
        '''
        sh_bdo_magic_number = b'xSH12A'
        with open(self.filename, "rb") as f:
            d1 = np.dtype([('magic', 'S6'),
    ...
    def read(self, detector, nscale=1):
        if self.test_version_0p6():
            if self.test_new_bdo_format():
                reader = _SHExperimentalBinaryReader0p7(self.filename)
            else:
...
class _SHExperimentalBinaryReader0p7:
    """
