...
@pytest.fixture()
def topas_no_config_path(main_dir) -> Generator[Path, None, None]:
    """Return path to Topas output file, input file is missing in the same directory"""
    yield main_dir / "res" / "topas" / "missing_config" / "fluence_bp_protons_xy.csv"


@pytest.fixture()
def topas_five_columns(main_dir) -> Generator[Path, None, None]:
    """Return path to Topas output file, input file is missing in the same directory"""
    yield main_dir / "res" / "topas" / "five_columns" / "PDD.csv"
...
def test_read(topas_minimal_output_path: Path):
    ...
    expected_data_mean = [
        0.00177, 0., 0., 0.00088, 0.00088, 0.00177, 0., 0.00088, 0.00266, 0.00088, 0.00177, 0.00177, 0.00088, 0., 0., 0.
    ]
    expected_data_std = [
        0.0053739, 0., 0., 0.00384373, .00475825, 0.00592593, 0., 0.00353209, 0.00642803, 0.00378469, 0.00628262,
...
def test_missing_config(topas_no_config_path: Path):
    """Test reading of Topas output file without any bins"""
    reader = TopasReaderFactory(str(topas_no_config_path)).get_reader()
    assert reader is not None

    reader = reader(str(topas_no_config_path))
    estimator = Estimator()
    reader.read_data(estimator)

    assert estimator.pages[0].dimension == 2


def test_five_columns(topas_five_columns: Path):
    """Test reading of Topas output file without any bins"""
    reader = TopasReaderFactory(str(topas_five_columns)).get_reader()
    assert reader is not None
